...
export default class BceSession {
    ...
    async acls(writable = null) {
        ...
        this.update_session_from_cookie();
        let result = await bce_rest_api.acl_list(get(user_session_token), writable);
    ...
    async chat_create(recipients, text) {
        console.log("BceSession.chat_create: here.")
        this.update_session_from_cookie();
        let result = await bce_rest_api.chat_create(get(user_session_token), recipients, text);
        return result;
    }
    
    async chat_list() {
        console.log("BceSession.chat_list: here.")
        this.update_session_from_cookie();
        let result = await bce_rest_api.chat_list(get(user_session_token));
        return result;
    }
    
    async device_list() {
        console.log("BceSession.device_list: here.")
        this.update_session_from_cookie();
        let result = await bce_rest_api.device_list(get(user_session_token));
        return result;
    }
    
  async device_update(uid, acl_id=null) {
        console.log("BceSession.device_update: here.")
        this.update_session_from_cookie();
        let result = await bce_rest_api.device_update(get(user_session_token), uid, acl_id);
        return result;
    }
