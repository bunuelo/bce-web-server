...
{#if $user_session_is_valid && $user_security_level >= 25}
  ...
  {#each assets as asset}
    <div style="display: inline-block; background: #7f7f7f; width: 272px; height:272px;">
        <img width="128px" height="128px" style="width: 128px; height: 128px; padding: 64px 0; display: block; margin: auto;" src="https://bce.center:8000/asset/download?session_token={$user_session_token}&name={asset.name}" alt="{asset.display_name}">
    </div>
  {/each}
